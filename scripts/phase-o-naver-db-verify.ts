/**
 * Phase O Naver DB verification — no PII / tokens in output.
 * Usage: pnpm exec tsx scripts/phase-o-naver-db-verify.ts
 */
...
async function main() {
  const naverCount = await prisma.socialAccount.count({
    where: { provider: SocialProvider.NAVER },
  });
  ...
  console.log('Phase O Naver DB verify');
