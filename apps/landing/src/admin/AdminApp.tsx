    ...
    <div className="admin-root">
      <div className="layout layout-wide">
        <header className="admin-nav card row">
          ...
          <Link to="/admin/payments" className={paymentHistoryActive ? 'nav-active' : undefined}>
            결제 내역
          </Link>
          <Link
            to="/admin/payment-settings"
            className={paymentSettingsActive ? 'nav-active' : undefined}
          >
            결제 설정
          </Link>
          <Link
            to="/admin/join-coin-policy"
    ...
    <div className="admin-page">
      ...
      <div className="card admin-section">
        ...
        <div className="dash-links" style={{ marginTop: 12 }}>
          ...
          <Link to="/admin/payment-settings" className="card dash-link-card">
            <span className="dash-link-title">결제 설정</span>
            <span className="dash-link-desc">Toss TEST 키 · 활성화</span>
          </Link>
          <Link to="/admin/join-coin-policy" className="card dash-link-card">
            <span className="dash-link-title">조인 설정</span>
            <span className="dash-link-desc">유형별 조인 생성 코인</span>
          </Link>
          <Link to="/admin/payments" className="card dash-link-card">
            <span className="dash-link-title">결제 내역</span>
            <span className="dash-link-desc">코인·프리미엄 결제 기록</span>
          </Link>
...
function PaymentSettingsPage() {
  ...
  async function save() {
    ...
    try {
      const body: UpdateAdminPaymentProviderSettingsRequest = {
        enabled,
        environment: isDevelopmentAdmin ? 'TEST' : environment,
      ...
      setClientKey(updated.clientKey ?? '');
      setSecretKey('');
      setMessage('저장되었습니다.');
    } catch (e) {
  ...
  return (
    <div className="admin-page">
      <div className="admin-page-header">
        <div>
          <h1 className="admin-page-title">결제 설정</h1>
          <p className="admin-page-desc">Toss Payments 연동 키와 활성화 상태를 관리합니다.</p>
        </div>
      </div>

      {error ? <p className="admin-error-banner">{error}</p> : null}
      {message ? <p className="admin-section-desc">{message}</p> : null}

      <section className="card admin-section" style={{ marginBottom: 12 }}>
        <strong className="admin-section-title">Provider: TOSS</strong>
        <p className="admin-section-desc">상태: {settings?.statusLabel ?? '—'}</p>
        {isDevelopmentAdmin ? (
          <p className="admin-section-desc">
            Development Admin에서는 TEST 환경만 저장할 수 있습니다. LIVE 결제 승인은 서버에서
            차단됩니다.
          </p>
        ) : null}
        ...
        <label style={{ display: 'block', marginBottom: 12 }}>
          ...
          <select
            value={isDevelopmentAdmin ? 'TEST' : environment}
            disabled={isDevelopmentAdmin}
        ...
        <label style={{ display: 'block', marginBottom: 12 }}>
          ...
          <input
            ...
            placeholder={settings?.hasSecretKey ? (settings.secretKeyMasked ?? '설정됨') : ''}
            autoComplete="new-password"
          />
        </label>
        {settings?.hasSecretKey && settings.secretKeyMasked ? (
          <p className="admin-section-desc">저장된 Secret: {settings.secretKeyMasked}</p>
        ) : settings?.hasSecretKey ? (
          <p className="admin-section-desc">저장된 Secret: 설정됨</p>
        ) : null}
        <div className="admin-page-actions" style={{ marginTop: 12 }}>
      ...
      <section className="card admin-section">
        <strong className="admin-section-title">상품</strong>
        <div className="admin-table-wrap">
          <table className="data-table">
            <thead>
              <tr>
...
function PaymentsAdminListPage() {
  ...
  useEffect(() => {
    void api<AdminPaymentListResponse>('/admin/payments')
      .then((res) => {
        setItems(res.items);
        setError(null);
      })
    ...
    <div className="admin-page">
      <div className="admin-page-header">
        <div>
          ...
          <p className="admin-page-desc">코인 충전·프리미엄 결제 기록을 조회합니다.</p>
        </div>
      </div>
                  ...
                  <tr key={row.id}>
                    ...
                    <td>
                      <button
                        ...
                        onClick={() =>
                          void api<AdminPaymentDetailDto>(`/admin/payments/${row.id}`).then(
                            setSelected,
                          )
                        }
                      >
      ...
      {selected ? (
        <section className="card admin-section" style={{ marginTop: 12 }}>
          <strong className="admin-section-title">결제 상세</strong>
...
function validateJoinCreationPricingDraft(draft: JoinCreationPricingPolicyDto): string | null {
  if (draft.baseMode === 'PAID' && draft.baseFeeCoinAmount <= 0) {
    return '기본 회원 유료 정책에는 1 Coin 이상의 생성 비용이 필요합니다.';
  }
  if (draft.ownerOverride === 'FIXED_FEE' && draft.ownerFixedFeeCoinAmount <= 0) {
    return '업주 별도 금액에는 1 Coin 이상이 필요합니다.';
  }
  if (draft.premiumOverride === 'FIXED_FEE' && draft.premiumFixedFeeCoinAmount <= 0) {
    return 'Premium 별도 금액에는 1 Coin 이상이 필요합니다.';
  }
  return null;
}
...
function JoinCoinPolicyPage() {
  ...
  async function save() {
    if (!draft) return;
    const validationError = validateJoinCreationPricingDraft(draft);
    if (validationError) {
    ...
    try {
      ...
      setDraft(updated);
      setPreview(previewData);
      setMessage('저장되었습니다. 변경된 설정은 변경 이후 새로 생성되는 조인부터 적용됩니다.');
    } catch (e) {
      setError(e instanceof Error ? e.message : '저장 실패');
    } finally {
      setBusy(false);
  ...
  if (!draft) {
    return (
      <div className="admin-page">
        {error ? <p className="admin-error-banner">{error}</p> : <p>불러오는 중…</p>}
      </div>
    );
  }
  ...
  return (
    <div className="admin-page">
      <div className="admin-page-header">
        <div>
          <h1 className="admin-page-title">조인방 생성 정책</h1>
      ...
      <section className="card admin-section" style={{ marginBottom: 12, maxWidth: 640 }}>
        <h3 className="admin-section-title">기본 회원</h3>
        <label>
          <input
            type="radio"
          ...
          <div style={{ marginTop: 8 }}>
            ...
            <input
              ...
              onChange={(e) =>
                setDraft({
                  ...draft,
                  baseFeeCoinAmount: Number(e.target.value.replace(/\D/g, '') || 0),
                })
              }
        ...
        <h3 className="admin-section-title" style={{ marginTop: 20 }}>업주 혜택</h3>
        ...
        <h3 className="admin-section-title" style={{ marginTop: 20 }}>Premium 혜택</h3>
        {(['INHERIT', 'FREE', 'FIXED_FEE'] as const).map((mode) => (
          <label key={mode} style={{ marginRight: 12 }}>
            <input
              type="radio"
      ...
      {preview ? (
        <section className="card admin-section" style={{ maxWidth: 640 }}>
          <h3 className="admin-section-title">현재 적용 예상</h3>
          <p>
            기본 회원: {preview.general.feeCoinAmount} Coin /{' '}
            {preview.general.feeKrw.toLocaleString('ko-KR')}원
          </p>
          <p>업주: {preview.owner.label}</p>
          <p>Premium: {preview.premium.label}</p>
          <p>업주 + Premium: {preview.ownerAndPremium.label}</p>
        </section>
      ) : null}
    </div>
  );
